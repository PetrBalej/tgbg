...
#' @param n integer or numeric: Ratio (< 1) or int to sample background points
...
#' @param weights numeric vector: optional weights per points
...
#'
#' @return named list: \emph{br}: RasterLayer (bias raster); \emph{bg}: sf (POINT/MULTIPOINT) (background points) with nested List of choosen \emph{sigma} levels
#'
#' @export

bg <- function(p, r, n = 0.1, sigma = c(0.1, 1, 2, 3), weights = NULL, output = c("br", "bg"), crs = NA, anisotropic = FALSE) {
    ...
    for (adj in sigma) {
        ...
        if (anisotropic) {
            ...
            d <- density.ppp(p.coords.ppp, weights = weights, varcov = diag(fa^2))
        } else {
            # sigma: isotropic kernel for projected crs (xy dimensions are equal)
            d <- density.ppp(p.coords.ppp, weights = weights, sigma = fa[1])
        }
